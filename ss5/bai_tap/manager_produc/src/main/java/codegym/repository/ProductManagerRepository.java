...
public class ProductManagerRepository implements IProductManagerRepository {
    ...
    @Override
    public Product update(Product produc) {
        Session session = null ;
        Transaction transaction = null ;
        try {
            session = sessionFactory.openSession();
            transaction = session.beginTransaction();
            Product origin = findById(produc.getId());
            origin.setId(produc.getId());
            origin.setName(produc.getName());
            origin.setPrice(produc.getPrice());
            origin.setDescription(produc.getDescription());
            origin.setProducer(produc.getProducer());
